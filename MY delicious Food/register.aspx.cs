...
public partial class register : System.Web.UI.Page
{
    ...
    {
        if (agree.SelectedItem.Text == "I Agree With Terms and Conditions")
        {


            string typ = "NotaMember";
            string memtyp = "Non";
            string filepath = FileUpload1.PostedFile.FileName;
            string filename = Path.GetFileName(filepath);
            FileUpload1.SaveAs(Server.MapPath("user_img/" + filename));
            ...
            else
            {
                ...
                cmd = new SqlCommand("insert into registration values('" + name_1.Text + "','" + uname.Text + "','" + gend + "','" + dob + "','" + email.Text + "','" + password1.Text + "','" + Confirmpwd.Text + "','" + occupation.Text + "','" + address.Text + "','" + telephone + "','" + mobileno.Text + "','" + agree + "','" + DropDownList1.SelectedItem.Text + "','" + TextBox1.Text + "','" + filename + "','" + typ + "','" + memtyp + "')", conn);
        ...
        {
            Response.Write("<script> alert ('You should agree with terms and conditions'); </script>");
